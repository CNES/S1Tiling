...
from s1tiling.libs.otbwrappers import (
        ...
        SelectGammaNaughtAreaBestCoverage, ApplyGammaNaughtRTCCalibration, ResampleDEM,
)
...
def LIA_task_s2() -> str:
    return file_db.tasklia_on_s2(tmp=False)
...
def LIA_task_s1(idx) -> str:
    return file_db.LIAtask(idx, tmp=False)
...
def given_pipeline_ortho_n_concat_GAMMA_AREA(pipelines, pipeline_ids, configuration) -> None:
    ...
    best_concat_ortho_gamma_area = pipelines.register_pipeline(
            ...
            'SelectGAMMA_AREA',
            product_required=GAMMA_AREA_product_required,
            is_name_incremental=True,
...
def given_pipeline_that_applies_GAMMA_AREA(pipelines, pipeline_ids) -> None:
    ...
    concat_S2  = pipeline_ids['concat']
    s2_gamma_naught_rtced = pipelines.register_pipeline(
            [ApplyGammaNaughtRTCCalibration], product_required=True,
...
def _check_registered_task(expectations, tasks, task_names, task2outfile_map) -> None:
    for req_taskname in task_names:
        assert req_taskname in task2outfile_map, f"{req_taskname} is not found in {task2outfile_map.keys()}"
...
def then_no_S2_LIA_image_is_required_s2(dependencies) -> None:
    ...
    assert LIA_task_s2() in previous.keys(), f'{LIA_task_s2()} is not found in computed dependencies {previous.keys()}'
...
def then_a_sin_LIA_task_is_registered_s2(tasks, dependencies) -> None:
    ...
    required, previous, task2outfile_map = dependencies
    # logging.info("tasks (%s) = %s", type(tasks), tasks.keys())
    assert isinstance(tasks, dict)
    assert len(tasks) >= 3
    # assert len(required) == len(expectations)
